...
public class Bank implements Serializable {
 ...
 public Checking createChecking(Customer cust,int accountNumb) throws java.util.InputMismatchException {
 ...
 System.out.println("\nInvalid entry. \nPlease note that menuselections, customer IDs and account numbers must be entered as integervalues. "
 + "\nDeposit and withdraw amounts must be entered asinteger or double values (EX: 100 or 100.00).");
 }
 Checking checking = new Checking(customer, accountNumber, balance,
transaction, accountType);
 return checking;
 }
 // method to create gold account
 public Gold createGold(Customer cust,int accountNumb) {
 Customer customer = null;
 int accountNumber = 0;
 float balance = 0;
 int transaction = 0;
...
accountType);
 ...
 public Regular createRegular(Customer cust,int accountNumb) {
 Customer customer = null;
 int accountNumber = 0;
 float balance = 0;
 int transaction = 0;
 ...
 Regular regular = new Regular(customer, accountNumber, balance,
transaction, accountType);
 return regular;
 }
 // method to request information about a customer and return customer
 public static Customer getCustomer() throws
java.util.InputMismatchException {
 ...
 list.add(Integer.toString(emptyAccount));
 ...
 list.add(Float.toString(averageBalance));
 ...
 Boolean addDeposit(int acc1,float dep) {
 Account acc;
 int accountNO = acc1;
 acc = find(accountNO);
 if (acc == null) {
 ...
 // method to withdraw
 Boolean subWithdraw(int acc1,float widthrawAmount) {
 Account acc;
 int accountNO = acc1;
 acc = find(accountNO);
 if (acc == null) {
 ...
 System.out.println("You were only able to withdraw: $" +
acc.balance);
 acc.balance = 0;
 }
 }
 ...
 public Account displayInfo(int accountNO) {
...
"\n");
 ...
 break;
 default:
 System.out.println("\nInvalid entry. Please ensure that yourselection is an integer from 1 to 10.");
 }
 } catch (java.util.InputMismatchException m) {
 System.out.println(
